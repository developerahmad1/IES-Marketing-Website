...
const LeadsPoup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://script.google.com/macros/s/AKfycbwxIZpciq3ORlyV55WUEhvL2ab5sZjCOvn1MWPtBKtJ7ygYP230BeGngTfoYByn8COG/exec",
        {
          method: "POST",
          headers: {
  ...
  `}
      ...
      <div
        ref={popupRef}
        className={`w-[90vw] max-w-[400px] h-[80vh] max-h-[300px] bg-white shadow-2xl fixed mx-auto mt-20 z-[10000000000000000] px-3 hover:border-[1px] rounded-2xl transition-all duration-[0.5s] hover:border-gray-900 hover:shadow-2xl ${
          shakePopup ? "shake" : ""
        }`}
      >
        <RxCross2
          className="text-[30px] text-black relative float-right cursor-pointer -mr-2 mt-1"
          onClick={closeLeadpoup}
        />
