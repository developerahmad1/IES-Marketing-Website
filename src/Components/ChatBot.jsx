...
const ChatBot = () => {
    const { isOpenChatBot, setIsOpenChatBot, openChatBot, closeChatBot, userMsg, setUserMsg, allMessages, setAllMessages } = useAppContext();
    ...
    const handleSendMessage = (e) => {
        ...
        if (userMsg.trim()) {
            ...
            setTimeout(() => {
                setAllMessages((prevallMessages) => {
                    if (prevallMessages[prevallMessages.length - 1]?.type !== "ies") {
                        return [
                            ...prevallMessages,
                            { msg: "This is Response of IES marketing.", type: "ies", time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }
                        ];
                    }
